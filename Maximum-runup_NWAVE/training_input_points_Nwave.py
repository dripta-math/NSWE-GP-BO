...
from FV_initial_file_Nwave import FV_2D_Nwave
...
amp_ratio_initial = np.zeros((num_cases,1));
t1_initial = np.zeros((num_cases,1));
t2_initial = np.zeros((num_cases,1));

inputs_all = np.zeros((num_cases,1,12));
...
for i in range(num_cases):
    ...
    amp_ratio_initial[i,0] = random.uniform(0.00001,1);
    t1_initial[i,0] = random.uniform(75.75, 90);
    t2_initial[i,0] = random.uniform(60, 74.25);
...
for i in range(num_cases):
    ...
    [runup_mat[i,0], time_all, runup_all] = FV_2D_Nwave(data_simul);
    
    if(i==0):
        runup_all_list = [ runup_all ];
        time_all_list = [ time_all ];
        f_FV_input = open('FV_input_Nwave.pickle', 'wb');
        pickle.dump([inputs_all, runup_mat, time_all_list, runup_all_list], f_FV_input);       #pickle.dump([we, rt], f_FV_input)
        f_FV_input.close()
    else:
        f_FV_output = open('FV_input_Nwave.pickle', 'rb');
