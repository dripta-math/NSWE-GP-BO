...
from IBV_LHS_file_Nwave import IBV_LHS_Nwave
from Roe_flux_file import Roe_Flux

def rhs(W, t, N, dx, eps, h, g, xc, Amp, amp_ratio, t1, t2):
    ...
    hi = np.minimum(hL, hR) ;        
    HLs = np.maximum( W[0:-1,0,:] - hL + hi, np.zeros((N-1,1)) ) ;   
    ...
    x0 = min(xc[:,0]);
    h_LBC  = IBV_LHS_Nwave(Amp, amp_ratio, t1, t2, t, h) ;
    ...
    rhs[N-1,:,:] = rhs[N-1,:,:] - np.array([ [Flux_RBC_1], [Flux_RBC_2] ]) ; # np.array([[ [Flux_RBC_1], [Flux_RBC_2] ]]) ;  
